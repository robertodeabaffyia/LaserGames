...
export default function ReservaForm({ onSuccess, onCancel }: ReservaFormProps) {
  ...
  // Load salas + pricing config once
  useEffect(() => {
    Promise.all([
      ...
      if (cfg) setPrecioSalaCompleta(cfg.precio_sala_completa ?? 0);
  ...
  const loadTurnos = useCallback(async (sala: string, dia: string) => {
    ...
    try {
      const res = await fetch(
        `/api/escape/turnos-disponibles?sala_id=${sala}&fecha=${dia}`
      );
  ...
  const canSubmit = !!contacto && !!horaInicio && !precioError && !saving;
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="rounded-lg bg-gray-800/60 border border-gray-700 px-4 py-2.5 text-sm">
        {precioError ? (
          ...
          <>
            Precio total: <span className="font-semibold text-white">{formatMoneda(precioPreview)}</span>
          </>
        )}
      </div>
